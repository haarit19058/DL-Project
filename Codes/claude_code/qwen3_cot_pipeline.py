...
class MemoryEfficientConfig:
    ...
    max_seq_length: int = 3072  # Reduced from 4096 for memory
    ...
    use_streaming: bool = True  # CRITICAL for large datasets
    
    # Multi-difficulty training
    difficulty_levels: List[str] = None  # Will be set in __post_init__
    
    # SFT - Curriculum Learning
    sft_samples_per_difficulty: Dict[str, int] = None  # Set in __post_init__
    sft_epochs: int = 2
    sft_batch_size: int = 1  # Reduced for memory
    sft_grad_accum: int = 16  # Increased to maintain effective batch
    sft_learning_rate: float = 2e-4
    sft_warmup_ratio: float = 0.1
    
    # GRPO - Memory optimized
    grpo_samples_per_difficulty: Dict[str, int] = None  # Set in __post_init__
    grpo_epochs: int = 2  # Reduced from 3
    grpo_batch_size: int = 1
    grpo_grad_accum: int = 16
    grpo_learning_rate: float = 3e-6  # More conservative
    grpo_num_generations: int = 3  # Reduced from 4
    grpo_max_completion_length: int = 1536  # Reduced for memory
...
class CurriculumSFT:
    ...
    def _format_sft_example(self, item: Dict) -> Optional[Dict]:
...
<|im_end|>"""
    ...
    def prepare_curriculum_dataset(self):
        ...
        train_dataset = self.data_loader.load_mixed_difficulty_dataset(
            ...
            format_fn=self._format_sft_example,
            shuffle_buffer_size=1000
        )
        
        # Create smaller eval dataset (only from A for speed)
        eval_samples = min(500, self.config.sft_samples_per_difficulty.get("A", 500) // 10)
        eval_dataset = self.data_loader.create_iterable_dataset(
            difficulty="A",
